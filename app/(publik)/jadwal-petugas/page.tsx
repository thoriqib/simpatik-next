...
const STATUS_DOT: Record<string, string> = {
    ...
    alpha: 'bg-red-400', terjadwal: 'bg-[#003580]',
};
...
export default async function JadwalPetugasPage({
    ...
    return (
        <>
            <div className="mb-5">
                <h2 className="text-xl font-bold text-gray-800">Jadwal Petugas Pelayanan</h2>
                <p className="text-sm text-gray-500 mt-1">Jadwal piket petugas Simpatik bulan {bulanNama}</p>
            </div>
            ...
            <form method="GET" className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 mb-5 flex flex-wrap gap-3 items-end">
                ...
                <button type="submit" className="bg-[#003580] text-white px-5 py-2 rounded-lg text-sm font-medium">Tampilkan</button>
            </form>

            {hariKerja.length > 0 ? (
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                    <div className="grid border-b border-gray-200 bg-[#003580] text-white text-sm font-semibold"
                    ...
                    {hariKerja.map((tgl) => {
                        ...
                        return (
                            <div key={tglStr}
                                className={`grid border-b border-gray-100 last:border-0 text-sm ${isToday ? 'bg-blue-50' : 'hover:bg-gray-50'} transition`}
                                style={{ gridTemplateColumns: `140px repeat(${shifts?.length ?? 1}, 1fr)` }}>
                                <div className="px-4 py-4 border-r border-gray-100">
                                    <div className={`font-semibold ${isToday ? 'text-[#003580]' : 'text-gray-700'}`}>
                                        {tgl.toLocaleDateString('id-ID', { weekday: 'short' })}
                                        {isToday && <span className="ml-1 text-xs bg-[#003580] text-white px-1.5 py-0.5 rounded">Hari ini</span>}
                                    </div>
                                    <div className="text-gray-500 text-xs mt-0.5">{tgl.toLocaleDateString('id-ID', { day: 'numeric', month: 'long' })}</div>
                                </div>
                                {shifts?.map((shift: ShiftPiket) => {
                                    const petugasShift = jadwalHari.filter((j) => j.shift_id === shift.id);
                                    return (
                                        <div key={shift.id} className="px-4 py-4 border-r border-gray-100 last:border-0">
                                                ...
                                                <div className="space-y-1.5">
                                                    {petugasShift.map((j) => (
                                                        <div key={j.id} className="flex items-center gap-2">
                                                            ...
                                                            <div>
                                                                <div className="text-xs font-medium text-gray-800 leading-tight">{j.nama_petugas}</div>
                                                                {j.status !== 'terjadwal' && (
                ...
                <div className="bg-white rounded-xl border border-gray-100 p-10 text-center text-gray-400">Tidak ada hari kerja di bulan ini.</div>
            )}

            <div className="mt-4 flex flex-wrap gap-4 text-xs text-gray-500">
                <span className="flex items-center gap-1.5"><span className="w-4 h-4 rounded-full bg-[#003580] inline-block" /> Terjadwal</span>
