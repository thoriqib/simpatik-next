...
export default async function JadwalPetugasSayaPage({ searchParams }: { searchParams: Promise<{ bulan?: string; tahun?: string }> }) {
    ...
    return (
        <>
            <h1 className="text-lg font-semibold text-gray-800 mb-4">Jadwal Piket Saya</h1>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-5">
                {[['Hadir', rekap.hadir, 'bg-green-100 text-green-700'], ['Izin', rekap.izin, 'bg-blue-100 text-blue-700'],
                  ['Sakit', rekap.sakit, 'bg-orange-100 text-orange-700'], ['Alpha', rekap.alpha, 'bg-red-100 text-red-700'],
                  ['Terjadwal', rekap.terjadwal, 'bg-gray-100 text-gray-600']].map(([label, val, color]) => (
                    <div key={label as string} className="bg-white rounded-xl border border-gray-100 p-4 text-center">
                        <div className={`text-2xl font-bold ${color} rounded-lg py-1 mb-1`}>{val}</div>
            ...
            <Card title={`Jadwal Bulan ${bulanNama}`}>
                    ...
                    return (
                        <div key={item.id} className="flex items-center justify-between py-3 border-b border-gray-100 last:border-0">
                            <div className="flex items-center gap-4">
                                <div className={`w-12 h-12 rounded-xl flex flex-col items-center justify-center text-center ${isToday ? 'bg-[#003580] text-white' : 'bg-gray-100 text-gray-700'}`}>
                                ...
                                <div>
                                    <div className="font-medium text-sm text-gray-800">{item.shift_piket?.nama_shift}</div>
                                    <div className="text-xs text-gray-500">{item.shift_piket?.jam_mulai}–{item.shift_piket?.jam_selesai}</div>
                                    {p && (
                ...
                }) : <div className="py-10 text-center text-gray-400 text-sm">Tidak ada jadwal piket untuk bulan ini</div>}
            </Card>
        </>
    );
}
