...
export function LinkSuksesCard({ token }: { token: string }) {
    const [disalin, setDisalin] = useState(false);
    const [link, setLink] = useState('');
    const t = useTranslations('permintaanData.linkCard');

    // Dibangun di client supaya otomatis pakai origin yang benar
    // (localhost saat dev, domain asli saat production) tanpa perlu
    // env var tambahan khusus untuk ini.
    useEffect(() => {
    ...
    return (
        <div className="mt-6 bg-paper-50 border border-paper-200 rounded-2xl p-5 text-left">
            <div className="text-xs text-navy-950/40 uppercase tracking-wide mb-2">{t('title')}</div>
