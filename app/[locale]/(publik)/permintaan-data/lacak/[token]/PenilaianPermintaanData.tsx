...
export function PenilaianPermintaanData({
    ...
    const t = useTranslations('permintaanData.rating');

    const LABEL_NILAI: Record<number, string> = {
        1: t('labels.1'), 2: t('labels.2'), 3: t('labels.3'), 4: t('labels.4'), 5: t('labels.5'),
    };

    function handleKirim(e: React.FormEvent) {
        e.preventDefault();
        if (!nilai) {
            setError(t('selectStarError'));
            return;
        }
        setError('');
    ...
    if (terkirim) {
        return (
            <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-5 text-center">
                ...
                <p className="text-sm font-medium text-emerald-700 mb-1">{t('thankYou')}</p>
