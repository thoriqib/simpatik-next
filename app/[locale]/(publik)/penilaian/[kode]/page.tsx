...
export default async function PenilaianPage({
    ...
    params: Promise<{ kode: string; locale: string }>;
    searchParams: Promise<{ sukses?: string }>;
}) {
    const { kode, locale } = await params;
    setRequestLocale(locale);
    const t = await getTranslations('penilaian');
    ...
    if (sukses === '1') {
        return (
            <div className="text-center py-10">
                ...
                <h2 className="text-xl font-bold text-navy-950">{t('thankYouTitle')}</h2>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-paper-200 p-6">
                <div className="flex items-center gap-4 p-4 bg-azure-500/10 rounded-xl mb-6">
                    ...
                    <div>
                        ...
                        <div className="text-sm text-navy-950/50">{t('antrian')}: {antrian.kode_antrian}</div>
