...
export default async function TiketPage({ params }: { params: Promise<{ kode: string; locale: string }> }) {
    const { kode, locale } = await params;
    setRequestLocale(locale);
    const t = await getTranslations('antrian.tiket');
        ...
        <div className="min-h-screen bg-navy-950 flex flex-col items-center justify-center p-4 pb-28 -mx-5 -my-8 print:bg-white print:block print:min-h-0 print:p-0 print:-mx-0 print:-my-0">
            ...
            <div className="bg-white rounded-3xl shadow-card w-full max-w-xs overflow-hidden print:shadow-none print:rounded-none print:mx-auto">
                <div className="bg-navy-950 text-white px-6 py-5 text-center print:bg-white print:text-navy-950 print:border-b-2 print:border-navy-950">
                    ...
                    <div className="text-white/40 text-xs mt-0.5 print:text-navy-950/60">{t('brand')}</div>
                </div>
                    ...
                    {antrian.status === 'menunggu' ? (
                        (menunggu ?? 0) > 0 ? (
                            <div className="bg-amber-500/10 rounded-xl px-4 py-3 text-center mb-3">
                                <div className="text-xs text-amber-600 font-medium">{t('queueAhead')}</div>
                            ...
                            <div className="bg-emerald-500/10 rounded-xl px-4 py-3 text-center mb-3">
                                <div className="text-sm font-medium text-emerald-600">{t('youAreNext')}</div>
                            </div>
                        )
                    ) : (
                        <div className="flex justify-center mb-3"><Badge status={antrian.status} /></div>
                    )}
                    <p className="text-xs text-navy-950/40 text-center print:hidden">{t('waitHint')}</p>
                </div>
                ...
                <div className="px-6 pb-6 flex flex-col items-center border-t border-dashed border-paper-200 pt-5">
                    ...
                    <p className="text-[10px] text-navy-950/40 mt-2 text-center">{t('scanHint')}</p>
                </div>
            </div>
            ...
            <div className="fixed bottom-6 left-0 right-0 flex justify-center gap-2.5 px-4 flex-wrap print:hidden">
                ...
                {/* [CATATAN] /display-antrian SENGAJA pakai next/link biasa —
                    di luar scope terjemahan (kios tampilan, tetap Indonesia). */}
                <NextLink href="/display-antrian"
