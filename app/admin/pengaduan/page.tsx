...
export default async function PengaduanListPage({ searchParams }: { searchParams: Promise<{ status?: string }> }) {
    ...
    return (
        <>
            <h1 className="text-lg font-semibold text-gray-800 mb-4">Daftar Pengaduan</h1>

            <div className="flex gap-2 mb-5 flex-wrap">
                {tabs.map((tab) => (
                    <Link key={tab.key} href={tab.key ? `/admin/pengaduan?status=${tab.key}` : '/admin/pengaduan'}
                        className={`px-4 py-2 rounded-lg text-sm font-medium transition ${(status ?? '') === tab.key ? 'bg-[#003580] text-white' : 'bg-white text-gray-600 border hover:bg-gray-50'}`}>
                ...
                <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {pengaduan && pengaduan.length > 0 ? pengaduan.map((p) => (
                            <tr key={p.id} className={`hover:bg-gray-50 ${p.status === 'baru' ? 'border-l-4 border-l-red-400' : ''}`}>
                                <td className="py-3 text-gray-500 text-xs">{new Date(p.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}</td>
                                <td className="py-3">
                                    <div className="font-medium text-gray-800">{p.subjek}</div>
                                    <div className="text-xs text-gray-400 mt-0.5">{p.isi_pengaduan.slice(0, 60)}</div>
                                </td>
                                <td className="py-3"><Badge status={p.status} /></td>
                                <td className="py-3 text-gray-500 text-sm">{p.profiles?.name ?? '—'}</td>
                                <td className="py-3">
                                    <Link href={`/admin/pengaduan/${p.id}`} className="text-xs font-medium bg-blue-50 text-blue-700 px-3 py-1.5 rounded-lg hover:bg-blue-100 transition">Detail →</Link>
                                </td>
                            </tr>
                        )) : <tr><td colSpan={5} className="py-10 text-center text-gray-400">Tidak ada pengaduan</td></tr>}
                    </tbody>
                </table>
            </Card>
        </>
