...
export default async function LaporanAntrianPage({ searchParams }: { searchParams: Promise<{ dari?: string; sampai?: string; status?: string }> }) {
    ...
    return (
        <>
            <h1 className="text-lg font-semibold text-gray-800 mb-4">Laporan Antrian</h1>
            ...
            <Card className="mb-5">
                <form method="GET" className="flex flex-wrap gap-3 items-end">
                    ...
                    <div>
                        ...
                        <select name="status" defaultValue={statusFilter} className="border border-gray-300 rounded-lg px-3 py-2 text-sm">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-5">
                <Card><div className="text-2xl font-bold text-gray-800">{total}</div><div className="text-sm text-gray-500">Total Antrian</div></Card>
                <Card><div className="text-2xl font-bold text-green-600">{selesai}</div><div className="text-sm text-gray-500">Selesai</div></Card>
                <Card><div className="text-2xl font-bold text-red-500">{batal}</div><div className="text-sm text-gray-500">Batal</div></Card>
                <Card><div className="text-2xl font-bold text-blue-600">{avgDurasi ? `${avgDurasi} mnt` : '-'}</div><div className="text-sm text-gray-500">Rata-rata Layanan</div></Card>
            </div>
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {antrian && antrian.length > 0 ? antrian.map((a) => (
                            <tr key={a.id} className="hover:bg-gray-50">
                                <td className="py-3 text-gray-500">{new Date(a.tanggal).toLocaleDateString('id-ID')}</td>
                                <td className="py-3 font-mono font-semibold text-blue-700">{a.kode_antrian}</td>
                                <td className="py-3">{a.nama_pengunjung}</td>
                                <td className="py-3 text-gray-600">{a.jenis_layanan?.nama_layanan}</td>
                                <td className="py-3 text-gray-600">{a.profiles?.name ?? '-'}</td>
                                <td className="py-3"><Badge status={a.status} /></td>
                            </tr>
                        )) : <tr><td colSpan={6} className="py-8 text-center text-gray-400">Tidak ada data antrian</td></tr>}
                    </tbody>
                </table>
            </Card>
        </>
