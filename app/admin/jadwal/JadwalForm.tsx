...
export function JadwalForm({ petugas, shifts }: { petugas: { id: string; name: string }[]; shifts: { id: number; nama_shift: string; jam_mulai: string }[] }) {
    ...
    return (
        <form action={formAction} className="flex flex-wrap gap-4 items-end">
            {state?.error && <div className="w-full bg-red-50 border border-red-200 text-red-700 px-3 py-2 rounded-lg text-sm">{state.error}</div>}
            <div className="flex-1 min-w-40">
                <label className="block text-sm font-medium text-gray-700 mb-1">Petugas</label>
                <select name="user_id" required className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
            ...
            <div className="flex-1 min-w-36">
                ...
                <select name="shift_id" required className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
            ...
            <div>
                ...
                <input type="date" name="tanggal" min={new Date().toISOString().slice(0, 10)} required className="border border-gray-300 rounded-lg px-3 py-2 text-sm" />
            </div>
