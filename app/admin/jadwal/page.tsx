...
export default async function JadwalPage({ searchParams }: { searchParams: Promise<{ bulan?: string; tahun?: string }> }) {
    ...
    return (
        <>
            <h1 className="text-lg font-semibold text-gray-800 mb-5">Jadwal Piket</h1>
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {jadwal && jadwal.length > 0 ? jadwal.map((j) => (
                            <tr key={j.id} className="hover:bg-gray-50">
                                ...
                                <td className="py-3">{j.shift_piket?.nama_shift} <span className="text-gray-400 text-xs">({j.shift_piket?.jam_mulai}–{j.shift_piket?.jam_selesai})</span></td>
                                <td className="py-3"><Badge status={j.status} /></td>
                                <td className="py-3 text-gray-600">{j.presensi?.[0]?.waktu_masuk ? new Date(j.presensi[0].waktu_masuk).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit', timeZone: 'Asia/Jakarta' }) : '-'}</td>
                                <td className="py-3 text-gray-600">{j.presensi?.[0]?.waktu_keluar ? new Date(j.presensi[0].waktu_keluar).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit', timeZone: 'Asia/Jakarta' }) : '-'}</td>
                                <td className="py-3">
                        ...
                        )) : <tr><td colSpan={7} className="py-8 text-center text-gray-400">Belum ada jadwal untuk bulan ini</td></tr>}
                    </tbody>
                </table>
            </Card>
